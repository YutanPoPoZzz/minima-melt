...
- **池（下部全幅）= MELTマクロ**: 画面下端に溶けた絵の具の池。表面は低周波でうねる発光の水面線（白線画＋紫グロー、fillは発光グラデのみ）。**池の表面を上下ドラッグすると水位が上下し、それがMELT量**（上げるほど世界が溶けていく）。%表示は池面の右端に。FXパネルの `#melt-slider` と双方向ミラー維持。高MELTで: 液塊がだらだらと下方へ垂れ下がって画面中央へ迫る・下縁のローブと雫が肥大し伸びる・池面が沸き立ち（DSPのbubble popに同期して泡が浮かんで弾ける）・壁全体の発光がゆらぐ。
- **蛇口 = ランダマイズ（GEN一括）**（2026-07-10追加: ユーザー「ランダマイズがどこにあるか分からない。蛇口とかどう？」）: 画面**左上、液塊に半分呑まれた白線画の蛇口**（左端から突き出た短いパイプ＋下向きスパウト＋十字ハンドル）。スパウトから液塊へ、細い光の滴がぽた…ぽた…と常時注がれている（新しいパターンが供給され続けているメタファ）。真下に小さなセリフラベル **GEN**。**タップ（pointerdown）= 全トラック一括ランダマイズ**: kick/hatC/hatO/clap/acid のsteps＋acidNotes/acidAcc/acidSlideを再抽選（エディタの各GENと同じ生成ロジックを全トラックに適用）→engineへ全送信→エディタ・点灯表示を更新。演出: ハンドルがくるっと回り、スパウトから太い光流が0.6秒ほどどばっと塊に注がれ、塊がぶるんと震える（kickEnv借用可）。ミュート状態やパラメータ・melt値は変えない。
- **4トラック = 実験室のガラス器具**（2026-07-10改訂: グラフィティグリフはユーザーNG→**化学製品モチーフ**へ。acid=薬品なので世界観も繋がる）: 左右の余白、互いに違う高さに、白線画のラボ器具が置かれている（浮遊禁止・棚にあるように定着）: KICK=**三角フラスコ（エルレンマイヤー）**、HATS=**試験管2本（ラック差し）**、CLAP=**ビーカー**、ACID=**スポイト（先端から雫が今にも落ちそう）**。各器具の中に紫に発光する液体（液面線＋微小な泡）が入っていて、ふちから溢れた雫が池まで細く垂れている。タップでそのトラックのエディタへ、ミュートで液の発光が消えて減光＋垂れが止まる。ラベルはセリフ大文字。
