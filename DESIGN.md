...
**構図の骨格宣言: 重力カスケードの正面壁。** 画面全体が「溶けていくグラフィティの壁」そのもの。上＝壁に描かれたバブルレターのピース、中＝そこから垂れ落ちる16本のドリップのカーテン、下＝溶けた絵の具が溜まる発光の池。視線は正面固定、支配する力は重力ひとつ。既存機の骨格（galaxy放射軌道/rain窓タブロー/drift一点透視/cityアイソメ/fissionオーブ+リング/smokeレコード盤）のどれとも異なること。**旧版のfission型テンプレ（中央シンボル+傾斜楕円リング16ノード+周回プレイヘッド+四隅の浮遊アイコン+左上縦ウィジェット）は使用禁止。シーンSVGコードを前バージョン・他機からコピーするのも禁止（ゼロから書く）。流用可はインフラのみ（grain/vignette/glowフィルタ定型、エディタ、audio配線）。**
...
- **ピース（上部中央、主役面積）= バブルレター「MELT」4文字のスロウアップ**: 画面上部の幅いっぱいに、ふくらんだ中空二重輪郭のバブルレター（手描き風・白線画＋紫グローのエコー線）。輪郭は常時とろりとwobbleし、各文字の下辺が糸を引いて垂れ続ける。**= 再生トグル**（**pointerdown束縛**、class `sun` 維持）。再生中はゆったり脈動し、キックで文字全体がぶるんと震えて池に波紋が走る。
- **16ステップ = ピースから垂れる16本のドリップ**: 文字群の下端から池へ向かって、画面幅に等間隔で垂れ下がる粘液の糸。消灯=かすかな細い糸。点灯=糸の先端が**重い雫にふくらんで紫に発光**（長さもランダムに違う）。**accentの点灯stepは雫がひと回り太く明るく、根元に小さな王冠**。**slideのstepは隣の糸と粘糸（カテナリーに垂れた光の糸）でねばっと連結**。タップでトグルは不要（打ち込みはエディタ）。
- **プレイヘッド = 粘性グロブ**: 光る飴状の塊がドリップ列を左→右に這って進む。移動のたび前の位置から**糸を引いて伸び→くびれて千切れ→ぷるんと合体**（蜂蜜の歩き方）。点灯ドリップを通過すると、そのドリップの雫が**千切れて池へ落下→重いスプラッシュ＋同心の粘った波紋**（波紋は速く広がらず、もったり減衰）。停止時は step0 のドリップ根元で休む（opacity 0.28）。
- **池（下部全幅）= MELTマクロ**: 画面下端に溶けた絵の具の池。表面は低周波でうねる発光の水面線（白線画＋紫グロー、fillは発光グラデのみ）。**池の表面を上下ドラッグすると水位が上下し、それがMELT量**（上げるほど世界が溶けていく）。%表示は池面の右端に。FXパネルの `#melt-slider` と双方向ミラー維持。高MELTで: ピースの文字がだらだらと下方へ崩れて判読不能に近づく・ドリップが太く長く本数の見た目も増す・池面が沸き立ち（DSPのbubble popに同期して泡が浮かんで弾ける）・壁全体の発光がゆらぐ。
- **4トラック = 壁に貼られた4つの小さなスロウアップ**（浮遊禁止 — 壁に定着）: 左右の余白、互いに違う高さに、小さなグラフィティグリフが「貼って」ある: KICK=**ワイルドスタイルの矢印**、HATS=**スパークル星**、CLAP=**「!」**、ACID=**王冠**。各々の下辺から自分の小さなペイントランが池まで細く垂れている。タップでそのトラックのエディタへ、ミュートで減光＋垂れが止まる。ラベルはセリフ大文字。
- 背景の壁: 微細なミスト浮遊粒＋ごく低opacityの古いタグ断片輪郭1〜2本＋乾いた古いペイントランの縦筋。旧作の「周回する彗星」的要素は使わない（この世界の動きはすべて下方向）。
