...
- シーン = **溶けるグラフィティ**（2026-07-10改訂: fission構図の焼き直し[中央オーブ+周回リング+左上の柱]はNG。グラフィティアートの語彙そのものがどろどろに溶けることで melt を表現する。ただし描画はあくまで白線画＋発光のみ＝「夜の壁に光で描かれたタグ」であって、スプレー塗装のリアル表現・塗り面は禁止）:
  - 中央 = **バブルレターの「M」スロウアップ**: グラフィティのthrow-up風にふくらんだ中空二重輪郭の「M」（白線画＋紫グロー、EB Garamondではなく手描き風の丸い一筆スタイル）。輪郭が常時ゆっくりwobbleし、下辺からペイントランが糸を引いて垂れ落ちる。= 再生トグル（**pointerdown束縛**）。再生中は脈動、キックでぶるんと膨らむ。
  - 16ステップ = 「M」を囲む**スプレー飛沫（splat）のリング**: 各ノードは小さな星型スプラット輪郭。点灯step=紫に発光するスプラット＋下に垂れるペイントラン。**accent stepはひと回り大きく明るく（王冠つき）**、**slide stepは前のstepから続く「垂れたペイントの筋」で連結**（壁を流れる絵の具の垂線）。
  - プレイヘッド = リングを周回する**スプレー缶**（白線画、小、drift car方式）。移動中はノズルから微細なミスト粒を後方に撒く。点灯ノード通過時に**スプレーバースト**: ミスト粒がふわっと広がる＋スプラットがびよんと伸びる＋小さな波紋。停止時は step0 にパーク（opacity 0.28）。
  - 4トラック = 周囲に浮かぶ4つの**溶けるグラフィティ・アイコン**（白線画＋発光ドットの心臓、galaxyの惑星相当）: KICK=**ワイルドスタイルの矢印**、HATS=**スパークル星**、CLAP=**エクスクラメーション「!」**、ACID=**王冠**。どれも輪郭の下側が垂れて溶けかけている。タップでそのトラックのエディタへ、ミュートで減光。ラベルはセリフ（EB Garamond）のまま。
  - MELT = 左上の**溶けるタグ文字「MELT」**: 手描きタグ風の小さな4文字（白線画）。**下ドラッグで溶解量**: 値が上がるほど文字が下方向にせん断・伸長し、各文字からペイントランが糸を引いて垂れる（100%でほぼ判読不能などろどろに）。%表示。FXパネルにミラースライダー `#melt-slider`。高MELTで: 「M」がだらりと垂れ下がる・スプラットリングが波打つ・背景のペイントラン増加・発光がゆらぐ。
  - 背景: 微細粒子の星野（スプレーミストの浮遊粒）＋時々壁を伝い落ちる**光のペイントラン**（垂直ストリーク、galaxyの彗星相当）＋ごく低opacityで巨大なタグの断片輪郭が1〜2本（うるさくしない）。
